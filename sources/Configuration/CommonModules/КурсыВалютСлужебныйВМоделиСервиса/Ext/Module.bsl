		...
		Попытка
			...
			Пока ВыборкаВалюты.Следующий() Цикл
				...
				Иначе 
					...
					// Сохранение установленного курса и кратности валюты на 1 января 1980 года.
					ДатаКурса = Дата("19800101");
					СтруктураКурса = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(ДатаКурса, Новый Структура("Валюта", ВыборкаВалюты.Ссылка));
					
					Запись = НаборЗаписей.Добавить();
					Запись.Валюта = ВыборкаВалюты.Ссылка;
					Запись.Период = ДатаКурса;
					Запись.Курс = СтруктураКурса.Курс;
					Запись.Кратность = СтруктураКурса.Кратность;
...
Процедура КопироватьКурсыВалюты(Знач КодВалюты) Экспорт
	...
	// Сохранение установленного курса и кратности валюты на 1 января 1980 года.
	ДатаКурса = Дата("19800101");
	СтруктураКурса = РегистрыСведений.КурсыВалют.ПолучитьПоследнее(ДатаКурса, Новый Структура("Валюта", ВалютаСсылка));
