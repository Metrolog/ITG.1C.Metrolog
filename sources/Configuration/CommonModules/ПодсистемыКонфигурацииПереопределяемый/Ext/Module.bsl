////////////////////////////////////////////////////////////////////////////////
// Подсистема "Базовая функциональность".
//  
////////////////////////////////////////////////////////////////////////////////

#Область ПрограммныйИнтерфейс

// Определяет список модулей библиотек и конфигурации, которые предоставляют
// основные сведения о себе: имя, версия, список обработчиков обновления
// а также зависимости от других библиотек.
//
// Состав обязательных процедур модуля см. в области ПрограммныйИнтерфейс
// общего модуля ОбновлениеИнформационнойБазыБСП.
//
// Параметры:
//  МодулиПодсистем - Массив - имена серверных общих модулей библиотек и конфигурации.
//                             Например: ОбновлениеИнформационнойБазыБРО - библиотека,
//                                       ОбновлениеИнформационнойБазыБП  - конфигурация.
//                    
// Примечание: модуль библиотеки стандартных подсистем ОбновлениеИнформационнойБазыБСП
// не требуется явно добавлять в массив МодулиПодсистем.
//
Процедура ПриДобавленииПодсистем(МодулиПодсистем) Экспорт
	
	// _Демо начало примера
	МодулиПодсистем.Добавить("МСОбновлениеИнформационнойБазы");
	// _Демо конец примера
	
	// ТехнологияСервиса
	МодулиПодсистем.Добавить("ОбновлениеИнформационнойБазыБТС");
	// Конец ТехнологияСервиса
	
	// ИнтернетПоддержкаПользователей
	МодулиПодсистем.Добавить("ОбновлениеИнформационнойБазыБИП");
	// Конец ИнтернетПоддержкаПользователей
	
	// СтандартныеПодсистемыИТГ
	МодулиПодсистем.Добавить("СтандартныеПодсистемыИТГОбновлениеИнформационнойБазыИТГ");
	// Конец СтандартныеПодсистемыИТГ

	// СтандартныеПодсистемыИТГ.СтруктурированныеРеквизиты
	МодулиПодсистем.Добавить("СтруктурированныеРеквизитыОбновлениеИнформационнойБазы");
	// Конец СтандартныеПодсистемыИТГ.СтруктурированныеРеквизиты

	// СтандартныеПодсистемыИТГ.Числа
	МодулиПодсистем.Добавить("ЧислаОбновлениеИнформационнойБазы");
	// Конец СтандартныеПодсистемыИТГ.Числа

	// МетрологическаяСлужба.ЕдиницыВеличин
	МодулиПодсистем.Добавить("ЕдиницыВеличинОбновлениеИнформационнойБазы");
	// Конец МетрологическаяСлужба.ЕдиницыВеличин

КонецПроцедуры

#КонецОбласти
