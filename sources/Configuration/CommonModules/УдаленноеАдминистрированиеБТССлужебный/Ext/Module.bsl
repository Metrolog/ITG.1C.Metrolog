...
Функция ПроксиСервисаУправляющегоПриложения(Знач ПарольПользователя = "") Экспорт
	...
	ПараметрыПодключения.АдресWSDL = ПараметрыПодключения.URL + "/ws/ManageApplication_" + СтрЗаменить(ИспользуемаяВерсия, ".", "_") + "?wsdl";
	...
	ПараметрыПодключения.ИмяПользователя = ПараметрыПодключения.UserName;
	ПараметрыПодключения.Пароль = ПараметрыПодключения.Password;
