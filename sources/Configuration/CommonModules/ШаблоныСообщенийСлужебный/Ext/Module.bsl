...
Функция СформироватьСообщение(ПараметрыОтправки) Экспорт
	...
	Если ПараметрыШаблона.ШаблонПоВнешнейОбработке Тогда
		Возврат СформироватьСообщениеВнешнейОбработкой(ПараметрыШаблона, СведенияОШаблоне);
	КонецЕсли;
	...
	// Заполняем параметры
	Сообщение = ЗаполнитьПараметрыСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, ПараметрыОтправки.Предмет);
...
Процедура ОпределитьСписокРеквизитовИВложений(ПараметрыШаблона, ДополнительныеПараметры)
	
	// Реквизиты
	МетаданныеОбъект = Метаданные.НайтиПоПолномуИмени(ДополнительныеПараметры.ПолноеИмяТипаНазначения);
	РеквизитыОбъектаНазначения = РеквизитыОбъектаНазначения(ПараметрыШаблона.Реквизиты, ДополнительныеПараметры.ПолноеИмяТипаНазначения);
	...
	Если МетаданныеОбъект <> Неопределено Тогда
		...
		Если ПустаяСтрока(ДополнительныеПараметры.Макет) Тогда
			РеквизитыПоМетаданнымОбъекта(РеквизитыОбъектаНазначения, МетаданныеОбъект,,, МетаданныеОбъект.Имя + ".");
		Иначе
			МакетСКД = МенеджерОбъекта.ПолучитьМакет(ДополнительныеПараметры.Макет);
			РеквизитыПоСКД(РеквизитыОбъектаНазначения, МакетСКД, МетаданныеОбъект.Имя);
		КонецЕсли;
		
		ОпределитьСписокПечатныхФорм(МетаданныеОбъект, ПараметрыШаблона);
	КонецЕсли;
	
	ШаблоныСообщенийПереопределяемый.ПриПодготовкеШаблонаСообщения(РеквизитыОбъектаНазначения, ПараметрыШаблона.Вложения, ДополнительныеПараметры.ПолноеИмяТипаНазначения, ДополнительныеПараметры);
	
	Если МетаданныеОбъект <> Неопределено Тогда
		МенеджерОбъекта.ПриПодготовкеШаблонаСообщения(РеквизитыОбъектаНазначения, ПараметрыШаблона.Вложения, ДополнительныеПараметры);
		Префикс = МетаданныеОбъект.Имя + ".";
	Иначе
		Префикс = ДополнительныеПараметры.ПолноеИмяТипаНазначения +"."; //ДополнительныеПараметры.Назначение + ".";
	КонецЕсли;
	...
	Для каждого РеквизитОбъектаНазначения Из РеквизитыОбъектаНазначения Цикл
		...
		КонецЕсли;
		Если ДополнительныеПараметры.РазворачиватьСсылочныеРеквизиты Тогда
...
Функция СформироватьСообщениеВнешнейОбработкой(ПараметрыШаблона, СведенияОШаблоне)
...
Функция РеквизитыОбъектаНазначения(Реквизиты, ПолноеИмяТипаНазначения)
	...
	Иначе
		...
		Представление = ПолноеИмяТипаНазначения;
	КонецЕсли;
...
Процедура ОпределитьСписокПечатныхФорм(МетаданныеОбъект, Знач ПараметрыШаблона)
		...
		Если ИсточникиКомандПечати.Найти(МетаданныеОбъект) <> Неопределено Тогда
			...
			Для каждого Вложение Из КомандыПечатиОбъекта Цикл
				Если НЕ Вложение.Отключена 
					...
					И ПустаяСтрока(Вложение.Обработчик)
...
Процедура ДобавитьВыбранныеПечатныеФормыВоВложения(ПараметрыОтправки, СведенияОШаблоне, Вложения, ПараметрыШаблона)
	...
	Для каждого ВложениеПечатнаяФорма Из СведенияОШаблоне.Вложения Цикл
		Если ВложениеПечатнаяФорма.Статус = "ПечатнаяФорма" 
			И ПараметрыШаблона.ВыбранныеВложения.Найти(ВложениеПечатнаяФорма.Идентификатор) <> Неопределено Тогда
...
Функция ПоместитьТабличныйДокументВоВременноеХранилище(ПечатнаяФорма, ПараметрыШаблоны, УникальныйИдентификатор)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		Если ЗаписьZipФайла <> Неопределено Тогда 
			...
			АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные, УникальныйИдентификатор); //, ЭтаФорма.УникальныйИдентификатор);
			НоваяСтрока = Результат.Добавить();
			НоваяСтрока.АдресВоВременномХранилище = АдресВоВременномХранилище;
...
Функция ПараметрыШаблона(Шаблон) Экспорт
	...
	Если ТипЗнч(Шаблон) = тип("ДанныеФормыСтруктура") Тогда
		...
		Для каждого ПечатныеФормыИВложения Из Шаблон.ПечатныеФормыИВложения Цикл
			Результат.ВыбранныеВложения.Добавить(ПечатныеФормыИВложения.Идентификатор);
		КонецЦикла;
	ИначеЕсли ТипЗнч(Шаблон) = Тип("СправочникСсылка.ШаблоныСообщений") Тогда
		...
		Если СведенияОШаблоне.Количество() > 0 Тогда
			...
			Результат.ВыбранныеВложения        = РезультатЗапроса[1].Выгрузить().ВыгрузитьКолонку("Идентификатор");
...
Функция ОписаниеПараметровШаблона() Экспорт
	...
	Результат.Вставить("ВыбранныеВложения", Новый Массив);
...
Функция ОпределитьПараметрыТекстаСообщения(ТекстСообщения)
	...
	Для каждого ЭлементаМассиваПараметров Из МассивПараметров Цикл
		...
		ЭлементСоответствия = НайтиЭлементСоответствия(СоответствиеПараметров, МассивРазобранныйПараметр, СтрокаФормата);
	КонецЦикла;
...
Функция НайтиЭлементСоответствия(СоответствиеПараметров, Знач МассивРазобранныйПараметр, СтрокаФормата)
	...
	Если ЭлементСоответствия = Неопределено Тогда
		Если МассивРазобранныйПараметр.Количество() > 1 Тогда
			...
			НайтиЭлементСоответствия(ВнутреннийЭлементСоответствия, МассивРазобранныйПараметр, СтрокаФормата)
		Иначе
	...
	Иначе
		Если МассивРазобранныйПараметр.Количество() > 1 Тогда
			МассивРазобранныйПараметр.Удалить(0);
			НайтиЭлементСоответствия(ЭлементСоответствия, МассивРазобранныйПараметр, СтрокаФормата)
		Иначе
			СоответствиеПараметров.Вставить(МассивРазобранныйПараметр[0] + СтрокаФормата, "");
		КонецЕсли;
	КонецЕсли;
...
Функция ЗаполнитьПараметрыСообщения(ПараметрыШаблона, ПараметрыТекстаСообщения, Предмет)
	...
	Если Предмет <> Неопределено 
		...
		Если ПараметрыТекстаСообщения[ИмяОбъекта] <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ПараметрыШаблона.Макет) Тогда
				ЗаполнитьРеквизитыПоСКД(ПараметрыТекстаСообщения[ИмяОбъекта], Предмет, ПараметрыШаблона.Макет);
			Иначе
...
Процедура ЗаполнитьРеквизитыПоСКД(Реквизиты, Предмет, ИмяМакета)
...
//
Функция ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(Знач ШаблонСтроки, ВставляемыеЗначения, Знач Префикс = "")
...
Функция ОбработатьОпределитьРеквизитыДляЗапросаПоМетаданным(ПараметрыОснования, МетаданныеОбъекта)
	...
	Для каждого ПараметрОснования Из ПараметрыОснования Цикл
		Если ТипЗнч(ПараметрОснования.Значение) = Тип("Соответствие") Тогда
			...
			КонецЕсли;
		ИначеЕсли МетаданныеОбъекта.Реквизиты.Найти(ПараметрОснования.Ключ) = Неопределено Тогда
			РеквизитНеНайден = Истина;
			Для каждого СтандартныеРеквизиты Из МетаданныеОбъекта.СтандартныеРеквизиты Цикл
				Если СтрСравнить(СтандартныеРеквизиты.Имя, ПараметрОснования.Ключ) = 0 Тогда
...
Функция ОбщиеРеквизиты(Реквизиты)
	...
	Если УзелРеквизитов = Неопределено Тогда
		...
		УзелРеквизитов.Представление = НСтр("ru = 'Общие реквизиты'");
	КонецЕсли;
...
Процедура ЗаполнитьПолучателейСообщения(ПараметрыОтправки, ПараметрыШаблона, Результат, МенеджерОбъекта)
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		...
		ШаблоныСообщенийПереопределяемый.ПриЗаполненииПочтыПолучателейВСообщении(Получатели, ПараметрыШаблона.ПолноеИмяТипаНазначения, ПараметрыОтправки.Предмет);
		Если МенеджерОбъекта <> Неопределено Тогда
				МенеджерОбъекта.ПриЗаполненииПочтыПолучателейВСообщении(Получатели, ПараметрыОтправки.Предмет);
		КонецЕсли;
	...
	Иначе
		...
		ШаблоныСообщенийПереопределяемый.ПриЗаполненииТелефоновПолучателейВСообщении(Получатели, ПараметрыШаблона.ПолноеИмяТипаНазначения, ПараметрыОтправки.Предмет);
		Если МенеджерОбъекта <> Неопределено Тогда
				МенеджерОбъекта.ПриЗаполненииТелефоновПолучателейВСообщении(Получатели, ПараметрыОтправки.Предмет);
		КонецЕсли;
...
Функция СформироватьПолучателейПоУмолчанию(Предмет, ТипКонтактнойИнформации);
	...
	Иначе
		...
		Получатели.Колонки.Добавить("Представление", Новый ОписаниеТипов("Строка"));
		Получатели.Колонки.Добавить("Контакт", Новый ОписаниеТипов("Строка"));
	КонецЕсли;
