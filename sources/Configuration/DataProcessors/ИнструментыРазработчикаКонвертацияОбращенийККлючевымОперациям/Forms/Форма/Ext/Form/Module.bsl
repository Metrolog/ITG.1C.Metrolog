...
Функция СписокВсехПодсистем()
	...
	ПолучитьПодсистемыИзДерева(СписокПодсистем, Метаданные.Подсистемы, "");
...
Функция СписокИспользуемыхПодсистем()
	...
	Если СтандартныеПодсистемы <> Неопределено Тогда
		...
		ПолучитьПодсистемыИзДерева(Результат, СписокПодсистем, "")
	КонецЕсли;
...
&НаСервере
Процедура ПолучитьПодсистемыИзДерева(СписокПодсистем, ВложенныеПодсистемы, ПутьКПодсистеме)
	...
	Если ВложенныеПодсистемы.Количество() > 0 Тогда 
		Для каждого Подсистема Из ВложенныеПодсистемы Цикл
			...
			ПолучитьПодсистемыИзДерева(СписокПодсистем, Подсистема.Подсистемы, ПутьКПодсистеме);
