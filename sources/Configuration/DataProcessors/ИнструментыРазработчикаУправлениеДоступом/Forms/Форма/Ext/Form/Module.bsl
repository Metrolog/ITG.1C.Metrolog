...
Функция ОписаниеНачальногоЗаполненияПрофиляНаВстроенномЯзыке(Профиль)
	...
	Для Каждого Строка Из Профиль.Назначение Цикл
		...
		Если Не ОбщегоНазначения.ЭтоСсылка(Тип) Тогда
			Продолжить;
		КонецЕсли;
		Назначение.Вставить(Тип, Истина);
	КонецЦикла;
	
	Если Назначение.Количество() > 1
	 Или Назначение.Получить(Тип("СправочникСсылка.Пользователи")) = Неопределено Тогда
		
		Текст = Текст + "
		|	// Переопределение назначения.";
		ДобавленныеНазначения = Новый Соответствие;
		Для Каждого Строка Из Профиль.Назначение Цикл
			Тип = ТипЗнч(Строка.ТипПользователей);
			Если Назначение.Получить(Тип) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ИмяТипаСсылки = ОбщегоНазначения.СтроковоеПредставлениеТипа(Тип);
			Если ДобавленныеНазначения.Получить(ИмяТипаСсылки) = Неопределено Тогда
				ДобавленныеНазначения.Вставить(ИмяТипаСсылки, Истина);
			КонецЕсли;
			Текст = Текст + "
