...
Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновлении()
	...
	Если НастройкиСоединения().УстанавливатьПодключениеНаСервере Тогда
		ПолучитьИОтобразитьИнформациюОДоступномОбновленииНаСервере(
			ПолучениеОбновленийПрограммыКлиент.ПараметрыКлиента());
	Иначе
...
Функция НовыйИнформацияОДоступномОбновлении(ФормаПомощника, ПараметрыКлиента)
	...
	Возврат ПолучениеОбновленийПрограммыКлиентСервер.ИнформацияОДоступномОбновлении(
		...
		ФормаПомощника.НастройкиСоединенияССерверами,
		ПараметрыКлиента);
	
КонецФункции

&НаСервере
Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновленииНаСервере(ПараметрыКлиента)
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Если Не ОбновлениеКомКонф.ДоступноОбновление И Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		ТекстСообщения = ТекстСообщения + НСтр("ru = 'Обновление не требуется. Установлена актуальная версия программы.'");
...
Процедура ЗагрузитьОбновлениеКонфигурации()
	...
	Иначе
		...
		Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(Обновление.ИмяПолученногоФайла, Ложь) Тогда
			...
			Исключение
				ПолучениеОбновленийПрограммыКлиент.ЗаписатьОшибкуВЖурналРегистрации(
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЕсли;
