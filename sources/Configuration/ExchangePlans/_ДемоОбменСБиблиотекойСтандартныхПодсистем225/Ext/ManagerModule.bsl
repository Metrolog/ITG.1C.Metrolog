...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Если ИдентификаторНастройки = "ТолькоОтправка" Тогда
		...
		ЗаголовокФормыПомощника = НСтр("ru='Отправка данных в 1С:Библиотека стандартных подсистем, версия 2.2.5 (настройка)'");
		ЗаголовокФормыУзла      = НСтр("ru='Отправка данных в 1С:Библиотека стандартных подсистем, версия 2.2.5'");
	...
	ИначеЕсли ИдентификаторНастройки = "ТолькоПолучение" Тогда
		...
		ЗаголовокФормыПомощника = НСтр("ru='Получение данных из 1С:Библиотека стандартных подсистем, версия 2.2.5 (настройка)'");
		ЗаголовокФормыУзла      = НСтр("ru='Получение данных из 1С:Библиотека стандартных подсистем, версия 2.2.5'");
	...
	ИначеЕсли ИдентификаторНастройки = "Двухсторонний" Тогда
		...
		ЗаголовокФормыПомощника = НСтр("ru='Синхронизация данных с 1С:Библиотека стандартных подсистем, версия 2.2.5 (настройка)'");
		ЗаголовокФормыУзла      = НСтр("ru='Синхронизация данных с 1С:Библиотека стандартных подсистем, версия 2.2.5'");
	КонецЕсли;
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	Если ИдентификаторНастройки = "ТолькоОтправка" Тогда
		ПоясняющийТекст = НСтр("ru = 'Позволяет отправлять данные в программу 1С:Библиотека стандартных подсистем, версия 2.2.5.'");
	ИначеЕсли ИдентификаторНастройки = "ТолькоПолучение" Тогда
		ПоясняющийТекст = НСтр("ru = 'Позволяет получать данные из программы 1С:Библиотека стандартных подсистем, версия 2.2.5.'");
	ИначеЕсли ИдентификаторНастройки = "Двухсторонний" Тогда
		ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные с программой 1С:Библиотека стандартных подсистем, версия 2.2.5.'");
	КонецЕсли;
	ПоясняющийТекст = ПоясняющийТекст + " " + НСтр("ru = 'Демонстрируется поддержка обратной совместимости, когда одна из программ разработана на базе редакции 2.3, а другая - на предыдущей версии 2.2.5.'");
...
Функция ПодробнаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	Иначе
		...
		Возврат "ПланОбмена._ДемоОбменСБиблиотекойСтандартныхПодсистем225.Форма.ПодробнаяИнформация";
