...
Функция НайтиПоПоказателю(Знач ПоказательСтепени, Знач ОснованиеСтепени = 10) Экспорт
	...
	Запрос.Текст = 
		...
		|   Ссылка
		|ИЗ
		|   Справочник.ПриставкиЕдиницыВеличин
		|ГДЕ
		|   (Основание = &Основание)
		|	И (Порядок = &Порядок)";
...
// Формирует данные выбора приставок по указанному основанию степени и диапазону показателей.
//
// Параметры:
//  ДанныеВыбора - СписокЗначений - Возвращаемый список значений выбора.
//  СтандартнаяОбработка - Булево
//  ОснованиеСтепениПриставки - Число - Основание степени
//  МинимальныйПоказательСтепениПриставки - Число - Минимальный показатель степени приставки. По умолчанию - -99.
//  МаксимальныйПоказательСтепениПриставки - Число - Максимальный показатель степени приставки. По умолчанию - 99.
// 
Процедура ОбработкаПолученияДанныхВыбораДляДиапазонаПоказателей(ДанныеВыбора, СтандартнаяОбработка,
	Знач ОснованиеСтепениПриставки,
	Знач МинимальныйПоказательСтепениПриставки = -99, Знач МаксимальныйПоказательСтепениПриставки = 99) Экспорт

	Перем Запрос;
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ПриставкиЕдиницыВеличин.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ПриставкиЕдиницыВеличин КАК ПриставкиЕдиницыВеличин
	|ГДЕ
	|	ПриставкиЕдиницыВеличин.Основание = &ОснованиеСтепениПриставки
	|	И ПриставкиЕдиницыВеличин.Порядок >= &МинимальныйПоказательСтепениПриставки
	|	И ПриставкиЕдиницыВеличин.Порядок <= &МаксимальныйПоказательСтепениПриставки
	|	И НЕ ПриставкиЕдиницыВеличин.ПометкаУдаления
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПриставкиЕдиницыВеличин.Порядок");

	Запрос.УстановитьПараметр("ОснованиеСтепениПриставки", ОснованиеСтепениПриставки);
	Запрос.УстановитьПараметр("МинимальныйПоказательСтепениПриставки", МинимальныйПоказательСтепениПриставки);
	Запрос.УстановитьПараметр("МаксимальныйПоказательСтепениПриставки", МаксимальныйПоказательСтепениПриставки);
...
Процедура ОбработкаПолученияПолейПредставления(Поля, СтандартнаяОбработка) 
	...
	Для Каждого ВариантОбозначения Из ЕдиницыВеличинКлиентСерверПовтИсп.ПолучитьВариантыОбозначенийЕдиницВеличин() Цикл
		Поля.Добавить(ЕдиницыВеличинКлиентСерверПовтИсп.ПолучитьРеквизитНаименованияПриставки(ВариантОбозначения));
		Поля.Добавить(ЕдиницыВеличинКлиентСерверПовтИсп.ПолучитьРеквизитНаименованияПриставки(ВариантОбозначения) + "ISO_80000_1");
	КонецЦикла;
