...
Процедура СформироватьИОтправить(Команда)
	...
	Иначе
		ПараметрыОтправки.ДополнительныеПараметры.ПреобразовыватьHTMLДляФорматированногоДокумента = Истина;
		ДополнительныеПараметры = Новый Структура("ПараметрыОтправки", ПараметрыОтправки);
		Оповещение = Новый ОписаниеОповещения("ПослеВопросаОбОткрытиеФормыСообщения", ЭтотОбъект, ДополнительныеПараметры);
...
Функция СформироватьСообщение(ПараметрыОтправки, ВидСообщения)
	...
	Если ВидСообщения = "ЭлектронноеПисьмо" Тогда
...
Процедура ОткрытьФормуСообщения(Знач ПараметрыОтправки)
	...
	Если Результат.СообщенияПользователю.Количество() > 0 Тогда
...
&НаКлиенте
Процедура ПослеВопросаОбОткрытиеФормыСообщения(Результат, ДополнительныеПараметры) Экспорт
