...
Процедура ПроверитьШаблон(Команда)
	...
	Если РезультатПроверкиШаблоны.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru = 'Неверный реквизит в шаблоне сообщения:'") + " ";
		Разделитель = "";
		Для каждого ОшибочныйРеквизит Из РезультатПроверкиШаблоны Цикл
			ТекстОшибки = ТекстОшибки + Разделитель + ОшибочныйРеквизит;
			Разделитель = ", ";
		КонецЦикла;
...
Процедура ВнешнейОбработкой(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		ИмяФормы = "ДополнительныеОтчетыИОбработки.ФормаВыбора";
		ОткрытьФорму("Справочник." + ИмяФормы, ПараметрыФормы, ЭтотОбъект,,,, Оповещение);
	КонецЕсли
...
Процедура ДобавитьПараметрВТекстСообщения(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Текст = "[" + ТекущиеДанные.Имя + ФорматВывода + "]";
		Если Объект.ТипТекстаПисьма = ПредопределенноеЗначение("Перечисление.СпособыРедактированияЭлектронныхПисем.HTML") Тогда
			Если ПустаяСтрока(Элементы.ТелоПисьмаВHTML.ВыделенныйТекст) Тогда
				ЗакладкаДляВставкиНачало = Новый ЗакладкаФорматированногоДокумента;
				ЗакладкаДляВставкиКонец = Новый ЗакладкаФорматированногоДокумента;
...
Процедура СформироватьСписокРеквизитовИПечатныхФорм()
	...
	ЗаполнитьДеревоРеквизитов(СписокРеквизитов, СведенияОШаблоне.Реквизиты, Ложь);
...
Процедура ЗаполнитьДеревоРеквизитов(Приемник, Источник, ЭтоОбщиеРеквизиты)
	ИндексКартинкиЭлемент = ?(ЭтоОбщиеРеквизиты, 1, 3);
	ИндексКартинкиУзел = ?(ЭтоОбщиеРеквизиты, 0, 2);
	...
	Для Каждого СтрокаДерева Из Источник.Строки Цикл
		...
		Если СтрокаДерева.Строки.Количество() > 0 Тогда
			...
			ЗаполнитьДеревоРеквизитов(НоваяСтрока, СтрокаДерева, ЭтоОбщиеРеквизиты);
		Иначе
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ДополнительныеОтчетыИОбработки") Тогда
		...
		Иначе
			...
			Если ЭлементНазначения = Неопределено Тогда
				...
				ОписаниеОшибки = НСтр("ru = 'Назначение %1 определенное в внешней обработке не найдено. Внешняя обработка не может быть подключена.'");
