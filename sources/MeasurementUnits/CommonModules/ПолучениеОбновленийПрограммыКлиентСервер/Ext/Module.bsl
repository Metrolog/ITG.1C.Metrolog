...
Функция ИнформацияОДоступномОбновленииВызовОперации(
	...
	СообщениеОбОшибкеПроверкиДоступности =
		ИнтернетПоддержкаПользователейКлиентСервер.ПроверитьURLДоступен(URLОперацииPing);
	
	Если Не ПустаяСтрока(СообщениеОбОшибкеПроверкиДоступности) Тогда
		Результат.ИмяОшибки = "ConnectError";
		Результат.Сообщение = НСтр("ru = 'Сервис недоступен.'");
