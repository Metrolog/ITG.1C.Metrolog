	...
	Для Каждого ФайлМодуля Из МассивМодулей Цикл
		...
		Пока НомерТекущейСтроки < КоличествоСтрок Цикл
			...
			Если (ПустаяСтрока(СимволПередВызовом) Или СимволПередВызовом = "(")
				...
				СтрокаОшибки = "%1 : %2";
				СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки, ПолноеИмяСтруктура.ПолноеИмяОбъекта, "Строка " + НомерТекущейСтроки);
...
Функция Подключаемый_ПроверитьВхождениеРолейИВидовДоступаВПрофили()
	...
	СписокОшибочныхОбъектов = Новый СписокЗначений;
	ЕстьИзменения = Ложь;
	Константы.ПараметрыОграниченияДоступа.СоздатьМенеджерЗначения().ОбновитьОписаниеСвойствВидовДоступа(ЕстьИзменения, Истина);
...
Функция ЭтоИсключение(ПолноеИмяОбъекта)
	...
	Если СтрНайти(ПолноеИмяОбъекта, "ИнструментыРазработчика") > 0 Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция Подключаемый_НекорректныеСвязиМеждуПодсистемами()
	...
	ОписаниеИсправления = НСтр("ru = 'Корректные зависимости необходимо отразить в функции ЗависимостиПодсистем
		|модуля объекта обработки ПервоеВнедрениеБСП, а так же в документации'");
	СтруктураОшибок =  Новый Структура(
		"ПредставлениеОшибки, ОписаниеИсправления, ОшибочныеДанные",
		ПредставлениеОшибки, ОписаниеИсправления, СписокОшибочныхОбъектов);
	Возврат СтруктураОшибок;
...
Процедура ОпределитьОшибкиУсловныхВызовов(ФайлМодуля, Знач СтрокаТекста, Ошибки, СписокИспользуемыхПодсистем, СтрокаУсловногоВызова)
	...
	Если ПолноеИмяСтруктура.ИмяОбъекта = "ОбщегоНазначенияКлиентСервер"
		Или ПолноеИмяСтруктура.ИмяОбъекта = "ИнструментыРазработчикаПроверкаБСППередСборкой" Тогда
		Возврат;
	КонецЕсли;
...
// в) в ролях права на реквизиты.
//
// Такие настройки сложно обнаружить, анализировать, объединять при доработках и когда требуется, объяснять в пользовательской
// документации. Кроме того, появляется жесткая зависимость от конкретной роли, чего рекомендуется избегать, так как такую роль
// крайне трудоемко или невозможно заменить при доработке конфигурации.
//
...
Функция НерекомендуемыеНастройкиПрав(ПапкаВыгрузки)
	...
	Файлы = НайтиФайлы(ПапкаВыгрузки, "Form.xml", Истина);
	...
	Для Каждого Файл Из Файлы Цикл
		...
		ИмяОбъекта = ИмяОбъекта(ПапкаВыгрузки, Файл.ПолноеИмя, ".Ext.Form.xml");
	...
	Для Каждого Файл Из Файлы Цикл
		...
		ПроверитьПраваНаРеквизиты(Текст, Фабрика, ИмяОбъекта);
	КонецЦикла;
...
// Для процедуры НерекомендуемыеНастройки.
&НаСервере
Процедура ПроверитьПраваНаРеквизиты(Текст, ОбъектXDTO, ИмяРоли)
	
	ТекстОшибок = "";
	
	УстанавливатьПраваДляНовыхОбъектов = СвойствоРоли(ТекстОшибок,
		ОбъектXDTO, "setForNewObjects");
	
	УстанавливатьПраваДляРеквизитовИТабличныхЧастейПоУмолчанию = СвойствоРоли(ТекстОшибок,
		ОбъектXDTO, "setForAttributesByDefault", Истина);
	
	НезависимыеПраваПодчиненныхОбъектов = СвойствоРоли(ТекстОшибок,
	...
	Если ЗначениеЗаполнено(ТекстОшибок) Тогда
		Текст = Текст + ЗаголовокОшибки + ТекстОшибок + Символы.ПС;
		Возврат;
	КонецЕсли;
		...
		Если Не ЭтоПоле Тогда // Это команда.
			Если (ПропускаемыйОбъект = Неопределено Или Не ПропускаемыйОбъект.View)
			   И ПраваНаПоле.View = Истина Тогда
				...
				ТекстОшибок = ТекстОшибок + ?(ЗначениеЗаполнено(ТекстОшибок), Символы.ПС, "")
