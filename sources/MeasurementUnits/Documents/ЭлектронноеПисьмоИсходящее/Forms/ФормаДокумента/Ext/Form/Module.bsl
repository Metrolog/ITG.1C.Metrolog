...
Процедура СписокПолучателейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НоваяСтрока Тогда
		Элемент.ТекущиеДанные.Представление = Адрес;
...
Процедура СформироватьСпискиПолучателейПисьма()
	...
	Для каждого СтрокаПолучатель Из Объект.ПолучателиПисьма Цикл
		НоваяСтрока = СписокПолучателей.Добавить();
		НоваяСтрока.ВариантОтправки = "Кому:";
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПолучатель);
	КонецЦикла;
	Для каждого СтрокаПолучатель Из Объект.ПолучателиКопий Цикл
		НоваяСтрока = СписокПолучателей.Добавить();
